...
class Module04 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test("Refactoring var to val") {
    ...
    def max(numbers: List[Int]): Int = numbers match {
      case Nil            => Int.MinValue
      case h :: Nil       => h
      case h :: n :: tail => max((if (h > n) h else n) :: tail)
    }
  ...
  def listShakespeareFiles(dirPath: String): Array[String] = {
    ...
    fileList.filter(_.getName.endsWith(".shkspr")).map(_.getName)
  }
  ...
  def firstLineOfFile(filePath: String): String = {
    val it: BufferedSource = scala.io.Source.fromFile(filePath)
    val contents = for (line <- it.getLines()) yield line
    val res = contents.toList.headOption.getOrElse("")
    it.close()
    res
  }
